...
const DisplayCartItem = ({ close }) => {
    const { noDiscountTotalPrice, totalPrice, totalQty } = useGolbalContext()
        ...
        <section className='bg-neutral-900 fixed top-0 bottom-0 right-0 left-0 bg-opacity-70 z-50'>
            <div className='bg-white w-full max-w-sm min-h-screen max-h-screen ml-auto'>
                <div className='flex items-center p-4 shadow-md gap-3 justify-between'>
                    ...
                    <button onClick={close} className='lg:block hidden'>
                        <IoClose size={25} />
                    </button>
                </div>
                                ...
                                <div className='bg-white rounded-lg p-4 grid gap-4 overflow-auto'>
                                    {
                                        cartItem[0] && (
                                            cartItem.map((item, index) => {
                                                return (
                                                    <div className='flex w-full gap-4' key={item?._id + "cartitemdisplay" + index}>
